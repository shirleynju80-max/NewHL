...
| 精选跟踪实时价=昨收 | Pages 静态站 `/api/quote` 会 SPA 回退成 HTML；`workers.dev` 境内常失败 → 回退 CSV 昨收。已用 `functions/api/quote.ts` 同源拉实时价；`liveQuote.ts` 会拒绝非 JSON 响应。验收：`curl -sI .../api/quote?code=513920` 须 `content-type: application/json` |
