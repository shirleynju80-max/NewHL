...
export function IndexDetailSectionNav({ hasBars, showSpread }: IndexDetailSectionNavProps) {
  ...
  return (
    <nav aria-label="页内导航" className="fin-subnav">
      ...
      {items.map((item) => (
        <a key={item.id} href={`#${item.id}`} className="fin-chip-filter fin-section-scroll">
          {item.label}
        </a>
      ))}
    </nav>
