...
/** 类内 Top N 入选行：标注来自全样本位和/或验证位 */
export type TopPickedRow = ScoredParamRow & {
  pickSlots: Array<"full" | "val">;
};
...
export type GridSearchOutcome = {
  ...
  maCross: TopPickedRow[];
  maCustom: TopPickedRow[];
  rsi: TopPickedRow[];
  boll: TopPickedRow[];
};
...
export const DEFAULT_PARAM_SEARCH: ParamSearchSnapshot = {
  ...
  bollModes: ["1d", "1w"],
  bollPeriods: [10, 20, 40, 60, 120],
  bollStd: [1.5, 2.0, 2.5, 3.0],
};

export function mergeParamSearch(
  p?: Partial<ParamSearchSnapshot>,
): ParamSearchSnapshot {
...
function splitChronoTrainVal(
  bars: OhlcBar[],
  trainRatio: number,
): { train: OhlcBar[]; val: OhlcBar[]; splitIdx: number } {
  const r = Math.min(0.95, Math.max(0.05, trainRatio));
  const splitIdx = Math.max(
    1,
    Math.min(bars.length - 1, Math.floor(bars.length * r)),
  );
  return {
    train: bars.slice(0, splitIdx),
    val: bars.slice(splitIdx),
    splitIdx,
  };
}
...
/** Top N：全样本最优 ⌈N/2⌉、验证集最优 ⌊N/2⌋（奇数时多 1 个全样本位）。 */
export function pickTopFullAndValRows(
  rows: ScoredParamRow[],
  topK: number,
): TopPickedRow[] {
  if (topK <= 0 || !rows.length) return [];
  const valSlots = Math.floor(topK / 2);
  const fullSlots = topK - valSlots;

  const byFull = [...rows].sort(
    (a, b) => b.excessReturnPct - a.excessReturnPct,
  );
  const byVal = [...rows].sort((a, b) => {
  ...
  const out: TopPickedRow[] = [];
  const findIdx = (r: ScoredParamRow) =>
    out.findIndex((x) => sameScoredParamRow(x, r));

  let fullAdded = 0;
  for (const r of byFull) {
    if (fullAdded >= fullSlots) break;
    const i = findIdx(r);
    if (i >= 0) {
      if (!out[i]!.pickSlots.includes("full")) out[i]!.pickSlots.push("full");
      fullAdded++;
      continue;
    }
    out.push({ ...r, pickSlots: ["full"] });
    fullAdded++;
  }

  let valAdded = 0;
  for (const r of byVal) {
    if (valAdded >= valSlots) break;
    const i = findIdx(r);
    if (i >= 0) {
      if (!out[i]!.pickSlots.includes("val")) {
        out[i]!.pickSlots.push("val");
        valAdded++;
      }
      continue;
    }
    out.push({ ...r, pickSlots: ["val"] });
    valAdded++;
  }
...
function sortRowsByValThenFull(rows: ScoredParamRow[]) {
  rows.sort((a, b) => {
    ...
    if (av != null && bv == null) return -1;
    if (av == null && bv != null) return 1;
    if (b.excessReturnPct !== a.excessReturnPct)
      return b.excessReturnPct - a.excessReturnPct;
    return b.score - a.score;
  });
}

export function sameScoredParamRow(
  a: ScoredParamRow,
  b: ScoredParamRow,
): boolean {
  return (
    ...
    a.paramVersion === b.paramVersion &&
    a.label === b.label
  );
}

export function strategyFamilyFromId(strategyId: string): StrategyFamily {
  const s = strategyId.toLowerCase();
  if (s.includes("ma_custom")) return "ma_custom";
  if (s.includes("rsi")) return "rsi";
  if (s.includes("boll")) return "boll";
  return "ma";
}

/** 用户自定义参数：与网格行同口径的 ScoredParamRow（含训练/验证超额） */
export function buildCustomBaselineDraft(
...
): { strategyId: string; params: EtfParams } {
  const strategyId = CUSTOM_BASELINE_STRATEGY_ID[kind];
  if (kind === "rsi") {
    return {
      strategyId,
      params: shellParamsRsi(
        values.rsiPeriod,
        values.rsiOverbought,
        values.rsiOversold,
      ),
    };
  }
  if (kind === "boll") {
    return {
      strategyId,
      params: shellParamsBoll(values.bollPeriod, values.bollStd),
    };
  }
  return {
    strategyId,
    params: shellParamsMa(values.maFast, values.maSlow),
  };
}

/** 与网格 Top 表同信号口径（RSI/布林支持日/周；MA 用 computeSignals） */
function scoreCustomOnBars(
  bars: OhlcBar[],
  params: EtfParams,
  strategyId: string,
  label: string,
  ...
  if (kind === "rsi") {
    ...
    const ser =
      mode === "1w"
        ? expandWeeklyRsiToDaily(bars, rv.period)
        : rsi(cl, rv.period);
    const sig = rsiSignalsFromSeries(ser, rv.overbought, rv.oversold);
    return scoreFromSignals(bars, sig, strategyId, label, "rsi", params);
  }
  if (kind === "boll") {
    ...
    if (mode === "1w") {
      const ex = expandWeeklyBollToDaily(bars, bv.period, bv.stdDev);
      upper = ex.upper;
      lower = ex.lower;
    } else {
      const bb = bollinger(cl, bv.period, bv.stdDev);
      upper = bb.upper;
      lower = bb.lower;
    }
    const sig = bollSignalsFromBands(cl, upper, lower);
    return scoreFromSignals(bars, sig, strategyId, label, "boll", params);
  }
  return scoreRow(bars, params, strategyId, label, "ma");
}

export function scoreCustomParamBaseline(
  bars: OhlcBar[],
  params: EtfParams,
  strategyId: string,
  label: string,
  ...
  const resolvedKind: CustomBaselineKind =
    kind ??
    (() => {
      const f = strategyFamilyFromId(strategyId);
      if (f === "rsi" || f === "boll") return f;
      return "ma";
    })();
  const trainRatio = options?.trainRatio ?? 0.7;
  const { train: barsTrain, val: barsVal } = splitChronoTrainVal(
    bars,
    trainRatio,
  );
  const scoreOn = (bx: OhlcBar[]) =>
    scoreCustomOnBars(bx, params, strategyId, label, resolvedKind, mode);
  const row = scoreOn(bars);
  if (!row) return null;
  return attachTrainValExcess(row, barsTrain, barsVal, scoreOn);
}
...
function buildSplitMeta(
  ...
  if (candidateCount > 0 && train.length < MIN_SLICE_BARS) {
    ...
    notes.push(
      `训练段仅 ${train.length} 根日K（建议≥${MIN_SLICE_BARS}），训练段指标可信度低。`,
    );
  } else if (candidateCount > 0 && val.length < MIN_SLICE_BARS) {
    credibility = "short_val_slice";
    notes.push(
      `验证段仅 ${val.length} 根日K（建议≥${MIN_SLICE_BARS}），「稳健优选」参考意义有限。`,
    );
  }
  if (
    candidateCount > 0 &&
    bars.length < WEAK_SAMPLE_BARS &&
    credibility === "ok"
  ) {
    credibility = "weak_sample";
    notes.push(
      `全样本仅 ${bars.length} 个交易日，低频换手机制下统计波动大，结论宜保守解读。`,
    );
  }
  return {
    trainRatio,
...
export type CustomBaselineKind = "rsi" | "boll" | "ma";

export const CUSTOM_BASELINE_STRATEGY_ID: Record<CustomBaselineKind, string> = {
  rsi: "str_rsi_mean_cn",
  boll: "str_boll_mr",
  ma: "str_ma_cn",
};

export function shellParamsMa(fast: number, slow: number): EtfParams {
  const vid = `ma_bt_${fast}_${slow}`;
  return {
    ma_variants: [{ variant_id: vid, fast, slow }],
    rsi_variants: [
      { variant_id: "rsi_shell", period: 14, overbought: 70, oversold: 30 },
    ],
...
export function shellParamsRsi(
  period: number,
  overbought: number,
  oversold: number,
): EtfParams {
...
export function shellParamsBoll(period: number, stdDev: number): EtfParams {
...
function expandWeeklyRsiToDaily(
  bars: OhlcBar[],
  period: number,
): (number | null)[] {
...
export function gridSearchTopParams(
  ...
  for (const fast of cfg.maCrossFast) {
    for (const slow of cfg.maCrossSlow) {
      ...
      if (!row) continue;
      maCrossRows.push(
        attachTrainValExcess(row, barsTrain, barsVal, (bx) =>
          scoreRow(bx, p, "str_ma_cn", label, "ma"),
        ),
      );
    }
  }
  ...
  for (const buyMa of cfg.maCustomBuyMa) {
    for (const profit of cfg.maCustomProfitPct) {
      for (const dd of cfg.maCustomDrawdownPct) {
        ...
        maCustomRows.push(
          attachTrainValExcess(row, barsTrain, barsVal, (bx) =>
            scoreRow(bx, p, "str_ma_custom", label, "ma_custom"),
          ),
        );
      }
    }
    ...
    for (const period of cfg.rsiPeriods) {
      for (const os of cfg.rsiOversold) {
        for (const ob of cfg.rsiOverbought) {
          ...
          if (!row) continue;
          rsiRows.push(
            attachTrainValExcess(row, barsTrain, barsVal, scoreRsiOn),
          );
        }
      }
    }
  ...
  for (const mode of cfg.bollModes) {
    for (const period of cfg.bollPeriods) {
      for (const std of cfg.bollStd) {
        ...
        if (!row) continue;
        bollRows.push(
          attachTrainValExcess(row, barsTrain, barsVal, scoreBollOn),
        );
      }
    }
  }
  ...
  for (const r of allMerged) {
    if (
      globalFullBest == null ||
      r.excessReturnPct > globalFullBest.excessReturnPct
    )
      globalFullBest = r;
  }
  for (const r of allMerged) {
    ...
    if (ev == null) continue;
    if (
      globalRobustBest == null ||
      ev > (globalRobustBest.excessValPct ?? -Infinity)
    )
      globalRobustBest = r;
  }

  const split = buildSplitMeta(
    bars,
    barsTrain,
    barsVal,
    trainRatio,
  ...
  return {
    ...
    globalRobustBest,
    globalFullBest,
    maCross: pickTopFullAndValRows(maCrossRows, topK),
    maCustom: pickTopFullAndValRows(maCustomRows, topK),
    rsi: pickTopFullAndValRows(rsiRows, topK),
    boll: pickTopFullAndValRows(bollRows, topK),
  };
}
