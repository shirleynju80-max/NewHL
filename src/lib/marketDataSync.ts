...
export function compareBarHistory(
  ...
  for (const d of overlapDates) {
    ...
    if (rel <= relTol) matched++;
      ...
      mismatchSamples.push(`${d} 收 ${a.close.toFixed(4)} ≠ ${b.close.toFixed(4)}`);
    }
  }
  ...
  if (!overlapDates.length) {
    ...
    messages.push(`重叠 ${overlapDates.length} 日收盘校验通过（容差 ±${(relTol * 100).toFixed(2)}%）。`);
  }
  return {
 ...
 */
export async function fetchBarsFromWebTemplate(code: string): Promise<RemoteBarsFetch> {
  const base = (import.meta as ImportMeta & { env?: Record<string, string> }).env?.VITE_QUOTE_WEB_URL;
  if (!base?.trim()) {
    return {
      ok: false,
      source: "none",
      detail: "未配置 VITE_QUOTE_WEB_URL。请在环境变量中设置行情网关（返回 { bars } JSON）。",
    };
  }
  try {
    const url = `${base.replace(/\/$/, "")}?code=${encodeURIComponent(code)}`;
    const r = await fetch(url, { mode: "cors", cache: "no-store" });
    if (!r.ok) return { ok: false, source: "web", detail: `Web HTTP ${r.status}` };
    const j = (await r.json()) as { bars?: OhlcBar[] };
    const bars = Array.isArray(j.bars) ? j.bars : null;
    if (!bars?.length) return { ok: false, source: "web", detail: "JSON 缺少非空 bars 数组" };
    return { ok: true, bars, source: "web" };
  } catch (e) {
    return { ok: false, source: "web", detail: e instanceof Error ? e.message : String(e) };
  }
}
 ...
 */
export async function fetchBarsFromApiTemplate(code: string): Promise<RemoteBarsFetch> {
  ...
  try {
    ...
    if (!r.ok) return { ok: false, source: "api", detail: `API HTTP ${r.status}` };
    const j = (await r.json()) as { bars?: OhlcBar[] };
    const bars = Array.isArray(j.bars) ? j.bars : null;
    if (!bars?.length) return { ok: false, source: "api", detail: "响应缺少非空 bars" };
    return { ok: true, bars, source: "api" };
  } catch (e) {
    return { ok: false, source: "api", detail: e instanceof Error ? e.message : String(e) };
  }
}
...
export async function tryWebThenApiBars(code: string, localBars?: OhlcBar[]): Promise<SyncRemoteResult> {
  const web = await fetchBarsFromWebTemplate(code);
  if (web.ok && web.bars) {
    const consistency = localBars?.length ? compareBarHistory(localBars, web.bars) : undefined;
    return { ...web, consistency };
  }
  const api = await fetchBarsFromApiTemplate(code);
  if (api.ok && api.bars) {
    const consistency = localBars?.length ? compareBarHistory(localBars, api.bars) : undefined;
    return { ...api, consistency };
  }
  const detail = [web.detail, api.detail].filter(Boolean).join(" → ");
  return { ok: false, source: "none", detail: detail || "Web 与 API 均未返回数据" };
}
