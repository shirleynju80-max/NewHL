 ...
 */
export function computeSignalsMaCustom(bars: OhlcBar[], params: EtfParams): Signal[] {
  const rule = params.ma_custom_rule;
  if (!rule || bars.length < rule.buyMaPeriod + 2) return bars.map(() => "HOLD");
  const closes = closesFromBars(bars);
...
export function indicatorWarmupBars(params: EtfParams, strategyId: string): number {
  if (usesMaCustomStrategy(strategyId) && params.ma_custom_rule) {
    return Math.min(500, Math.max(40, params.ma_custom_rule.buyMaPeriod * 3 + 20));
  }
...
export function computeSignals(bars: OhlcBar[], params: EtfParams, strategyId: string): Signal[] {
  if (usesMaCustomStrategy(strategyId)) return computeSignalsMaCustom(bars, params);
