...
import { bondAnchorLabel, bondAnchorShortLabel, resolveBondAnchorForIndex } from "../lib/bondAnchor";
...
import { buildIndexSpreadRows, indexSeriesForMode, indexShowsSpread } from "../data/indexCsv";
...
function signedClass(v: number | null | undefined) {
 if (typeof v !== "number" || !Number.isFinite(v) || v === 0) return "text-[var(--fin-text)]";
...
function PercentileMeter({ label, value, percentile }: { label: string; value: number | null; percentile: number | null }) {
 ...
 <p className="mt-1 font-mono text-xl font-semibold text-[var(--fin-text)]">{formatPct(value)}</p>
 </div>
 <div className="text-right">
 <p className="text-xs fin-muted-text">历史分位</p>
 <p className="mt-1 font-mono text-lg font-semibold text-[var(--fin-blue)]">{formatPct(pct)}</p>
 </div>
 </div>
 <div className="mt-3 h-2 overflow-hidden rounded-full bg-zinc-200">
 <div className="h-full rounded-full bg-[var(--fin-blue-soft)]0" style={{ width: `${pct ?? 0}%` }} />
 </div>
...
export function IndexDetailPage() {
 ...
 const { getIndex, getEtf, indices, etfProducts, bondByDate, publicCsvAutoLoading } = useDataSource();
 ...
 return def.meta.market === "H" ? hkBondAnchor : resolveBondAnchorForIndex(def);
 }, [def, hkBondAnchor]);
 ...
 if (!ix) return [];
 const series = indexSeriesForMode(ix.bars, "tri").filter((p) => Number.isFinite(p.value) && p.value > 0);
 ...
 () =>
 def && shouldShowSpreadModule ? buildIndexSpreadRows(def, bondByDate, effectiveBondAnchor) : [],
 ...
 const primaryProduct = def ? primaryProductForIndex(etfProducts, def.meta.index_code) : undefined;
 ...
 <p className="mt-1 font-mono text-sm fin-muted-text">{def.meta.index_code}</p>
 <p className="mt-2 text-xs fin-muted-text">指数研究 · 图表与绩效仅使用 index_bars 全收益序列</p>
 {primaryProduct ?
 <p className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
 <span className="fin-muted-text">主跟踪：</span>
 <Link
 to={`/etf/${encodeURIComponent(primaryProduct.code)}`}
 ...
 <IndexDetailSectionNav hasBars={hasBars} showSpread={shouldShowSpreadModule} />
 ...
 <section id="section-meta" className="fin-section-scroll fin-panel space-y-3 p-6">
 <h2 className="text-base font-semibold text-[var(--fin-text)]">基本信息</h2>
 <dl className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5">
 <div>
 <dt className="text-xs fin-muted-text">指数名称</dt>
 <dd className="mt-1 font-medium text-[var(--fin-text)]">{def.meta.name}</dd>
 </div>
 <div>
 <dt className="text-xs fin-muted-text">代码</dt>
 <dd className="mt-1 font-mono text-[var(--fin-text)]">{def.meta.index_code}</dd>
 </div>
 <div>
 <dt className="text-xs fin-muted-text">基期</dt>
 <dd className="mt-1 font-mono text-[var(--fin-text)]">{baseDate ?? "—"}</dd>
 </div>
 <div>
 <dt className="text-xs fin-muted-text">启用日期</dt>
 <dd className="mt-1 font-mono text-[var(--fin-text)]">{launchDate ?? "—"}</dd>
 </div>
 <div>
 <dt className="text-xs fin-muted-text">加权方式</dt>
 <dd className="mt-1 text-[var(--fin-text)]">{weightingMethod ?? "—"}</dd>
 </div>
 </dl>
 ...
 {hasBars ?
 <>
 <section id="section-perf" className="fin-section-scroll fin-panel p-6 shadow-sm">
 <h2 className="text-lg font-semibold text-[var(--fin-text)]">绩效（全样本）</h2>
 ...
 <section id="section-chart" className="fin-section-scroll fin-panel overflow-hidden shadow-sm">
 ...
 quickRange === r.id ? "bg-blue-50 text-blue-900" : "text-slate-900 hover:bg-fin-panel-muted"
 }`}
 >
 {r.label}
 </button>
 ...
 <div className="flex shrink-0 items-center gap-2 rounded-md border border-fin-border bg-white px-2.5 py-2 font-mono text-sm fin-muted-text">
 <input
 type="date"
 value={perfStartDate ?? ""}
 min={firstPerfDate}
 ...
 className="w-[128px] bg-transparent fin-muted-text outline-none"
 />
 <span className="text-[var(--fin-dim)]">至</span>
 <input
 type="date"
 value={perfEndDate ?? ""}
 ...
 <p className="shrink-0 text-sm text-[var(--fin-dim)]">更新日期: {latestDate}</p>
 ...
 className="w-full max-w-[280px] rounded-md border border-fin-border bg-fin-panel-muted px-3 py-2 text-sm text-slate-700"
 >
 ...
 <section id="section-metrics" className="fin-section-scroll fin-panel overflow-x-auto shadow-sm">
 <table className="w-full min-w-[920px] table-fixed text-xs text-slate-950 xl:min-w-0">
 <thead>
 <tr className="border-b border-fin-border bg-slate-100/80 text-sm font-semibold">
 <th className="w-[160px] px-3 py-3 text-left"></th>
 <th className="border-l border-fin-border px-2 py-3 text-center" colSpan={RETURN_WINDOWS.length}>阶段性收益（%）</th>
 <th className="border-l border-fin-border px-2 py-3 text-center" colSpan={ANNUAL_WINDOWS.length}>年化收益（%）</th>
 <th className="border-l border-fin-border px-2 py-3 text-center" colSpan={ANNUAL_WINDOWS.length}>年化波动率（%）</th>
 </tr>
 <tr className="border-b border-fin-border bg-slate-100/80 font-semibold">
 <th className="px-3 py-3 text-left font-mono">{latestDate}</th>
 {RETURN_WINDOWS.map((id) => (
 <th key={id} className="border-l border-fin-border px-2 py-3 text-center">{METRIC_WINDOWS.find((w) => w.id === id)?.label}</th>
 ))}
 {ANNUAL_WINDOWS.map((id) => (
 <th key={`ann-${id}`} className="border-l border-fin-border px-2 py-3 text-center">{METRIC_WINDOWS.find((w) => w.id === id)?.label}</th>
 ))}
 {ANNUAL_WINDOWS.map((id) => (
 <th key={`vol-${id}`} className="border-l border-fin-border px-2 py-3 text-center">{METRIC_WINDOWS.find((w) => w.id === id)?.label}</th>
 ))}
 </tr>
 </thead>
 ...
 {ANNUAL_WINDOWS.map((id) => (
 <td key={`vol-${id}`} className="px-2 py-3 text-center font-mono">{formatPct(row.windows[id].annualVolPct)}</td>
 ))}
 </tr>
 ))}
 </tbody>
 ...
 <section id="section-spread" className="fin-section-scroll fin-panel space-y-4 p-6 shadow-sm">
 <div>
 <h2 className="text-lg font-semibold text-[var(--fin-text)]">红利配置参考：股息率、利差与价格</h2>
 ...
 {def.meta.market === "H" ?
 <div className="mt-3">
 <BondAnchorToggle value={hkBondAnchor} onChange={setHkBondAnchor} />
 <p className="mt-2 text-[11px] fin-muted-text">切换基准后，利差、分位与下图将按所选国债收益率重新计算。</p>
 </div>
 : null}
 </div>
 {spreadRows.length > 0 ?
 ...
 <p className="mt-1 font-mono text-2xl font-semibold text-[var(--fin-text)]">{formatPct(latestSpreadPoint?.divYieldPct)}</p>
 <p className="mt-2 text-xs fin-muted-text">股息率越高，红利资产的现金回报吸引力越强</p>
 </div>
 <div className="rounded-2xl border border-fin-border bg-fin-panel-muted/70 p-4">
 <p className="text-xs fin-muted-text">当前股债利差</p>
 <p className="mt-1 font-mono text-2xl font-semibold text-[var(--fin-text)]">{formatPct(latestSpreadPoint?.spreadPct)}</p>
 <p className="mt-2 text-xs fin-muted-text">利差越高，红利相对债券的补偿越充分</p>
 </div>
 </div>
 ...
 return <span className={active ? "fin-muted-text" : "text-[var(--fin-dim)] line-through"}>{String(value)}</span>;
 }}
 />
 <Line
 yAxisId="right"
 ...
 <section id="section-products" className="fin-section-scroll fin-panel overflow-x-auto p-6">
 <h2 className="text-lg font-semibold text-[var(--fin-text)]">跟踪产品</h2>
 <p className="mt-1 text-xs fin-muted-text">
 同指数候选 {indexProducts.length} 只（主跟踪 {indexProducts.filter((p) => p.isPrimary).length} · 参考{" "}
 {indexProducts.filter((p) => !p.isPrimary).length}）。规模与费率来自基金公告；仅<strong>主跟踪</strong>提供策略回测 /
 盘中监控，参考产品见产品详情。
 </p>
