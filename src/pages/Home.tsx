    ...
    <div className="rounded-2xl border border-zinc-100 bg-zinc-50/40 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-baseline gap-2">
          ...
          <span className="text-xs text-zinc-400">{items.length} 只</span>
        </div>
        {items.length > 0 ?
          <button
            type="button"
      ...
      {items.length === 0 ? (
        <p className="mt-2 text-sm text-zinc-400">暂无</p>
      ) : (
              ...
              >
                <label className={`flex cursor-pointer items-start pt-0.5 ${disableNew ? "opacity-50 cursor-not-allowed" : ""}`}>
                  <input
                    type="checkbox"
                    disabled={disableNew}
                    className="mt-0.5 h-3.5 w-3.5 rounded border-zinc-300 text-indigo-600 accent-indigo-600"
                    checked={checked}
                ...
                <Link
                  to={`/etf/${e.meta.code}`}
                  className="group min-w-0 flex-1 rounded px-0.5 py-0 transition hover:bg-zinc-50"
                >
                  <div className="min-w-0">
                    <p className="font-mono text-[9px] text-zinc-400">{e.meta.code}</p>
                    <p className="mt-0.5 text-xs font-semibold leading-snug text-zinc-900 group-hover:text-indigo-700 line-clamp-2">
                      {e.meta.name}
                    </p>
                    <p className="mt-0.5 text-[9px] text-zinc-500">
                      成立（首交易日）<span className="font-mono text-zinc-600">{barStart}</span>
                    </p>
                  </div>
                </Link>
              </li>
  ...
  const summaryCards = useMemo<SummaryCard[]>(() => {
    ...
    if (y5Rows.length > 0) {
      ...
      cards.push(
        {
          title: "近5年优选",
          value: `${best.code} ${best.block.annualReturnPct}%`,
        ...
        {
          title: "风险提示",
          value: `${worstDd.code} 回撤 ${worstDd.block.maxDrawdownPct}%`,
          note: "近5年最大回撤最高标的",
          href: "#overview-metrics",
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="rounded-3xl border border-zinc-100 bg-white p-8 shadow-sm">
        ...
        {compareCodes.length < 2 && (
          <p className="mt-6 text-sm text-zinc-500">请在 ETF 标的池中勾选至少 2 个标的（最多 {MAX_COMPARE} 只）。</p>
        )}
        ...
        {compareResult && (
          <div className="mt-8 space-y-8">
            <div id="overview-metrics">
              ...
              <div className="mt-3 overflow-x-auto rounded-xl border border-zinc-100">
                <table className="w-full table-fixed text-left text-[11px] leading-tight sm:text-xs">
                  ...
                  <thead className="bg-zinc-50 text-[10px] font-semibold text-zinc-600 sm:text-[11px]">
                    <tr>
                      <th className="px-1 py-2 text-left">
                        <button type="button" onClick={() => toggleSort("code")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                          代码 {sortState.key === "code" ? (sortState.dir === "asc" ? "↑" : "↓") : ""}
                        </button>
                      </th>
                      <th className="px-1 py-2 text-left">
                        <button type="button" onClick={() => toggleSort("name")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                      ...
                      <th className="px-0.5 py-2 text-right">
                        <button type="button" onClick={() => toggleSort("days")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                          日 {sortState.key === "days" ? (sortState.dir === "asc" ? "↑" : "↓") : ""}
                        </button>
                      </th>
                      <th className="px-0.5 py-2 text-right" title="区间首尾收盘累计涨跌">
                        <button type="button" onClick={() => toggleSort("totalReturnPct")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                          区间收益% {sortState.key === "totalReturnPct" ? (sortState.dir === "asc" ? "↑" : "↓") : ""}
                        </button>
                      </th>
                      <th className="px-0.5 py-2 text-right" title="由区间复利按 252 交易日年化">
                        <button type="button" onClick={() => toggleSort("annualReturnPct")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                          年化% {sortState.key === "annualReturnPct" ? (sortState.dir === "asc" ? "↑" : "↓") : ""}
                        </button>
                      </th>
                      <th className="px-0.5 py-2 text-right">
                        <button type="button" onClick={() => toggleSort("maxDrawdownPct")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                          回撤% {sortState.key === "maxDrawdownPct" ? (sortState.dir === "asc" ? "↑" : "↓") : ""}
                        </button>
                      </th>
                      <th className="px-0.5 py-2 text-right">
                        <button type="button" onClick={() => toggleSort("annualVolPct")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                          波动% {sortState.key === "annualVolPct" ? (sortState.dir === "asc" ? "↑" : "↓") : ""}
                        </button>
                      </th>
                      <th className="px-0.5 py-2 text-right">
                        <button type="button" onClick={() => toggleSort("sharpeLike")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                          夏普 {sortState.key === "sharpeLike" ? (sortState.dir === "asc" ? "↑" : "↓") : ""}
                        </button>
                      </th>
                      <th className="px-0.5 py-2 text-right">
                        <button type="button" onClick={() => toggleSort("calmarLike")} className="inline-flex items-center gap-1 hover:text-zinc-900">
                  ...
                  <tbody className="divide-y divide-zinc-100">
                    {overviewRowsSorted.map((r) => {
                      ...
                      if (!seg) {
                        return (
                          <tr key={r.code} className="hover:bg-zinc-50/80">
                            <td className="px-1 py-1.5 font-mono text-zinc-700">{r.code}</td>
                            <td className="px-1 py-1.5 text-zinc-800">{r.name}</td>
                            <td className="px-1 py-1.5 text-zinc-600">{currentWindowLabel}</td>
                            <td colSpan={8} className="px-1 py-1.5 text-zinc-500">
                              有效样本不足（需约 ≥20 个交易日）
                            </td>
                          </tr>
                        );
                      ...
                      return (
                        <tr key={`${r.code}-${windowKey}`} className="hover:bg-zinc-50/80">
                          <td className="px-1 py-1.5 font-mono text-zinc-700">{r.code}</td>
                          <td className="max-w-[5.5rem] truncate px-1 py-1.5 text-zinc-800" title={r.name}>
                            {r.name}
                          </td>
                          <td className="px-1 py-1.5 text-zinc-600">{currentWindowLabel}</td>
                          <td
                            className="px-0.5 py-1 align-top font-mono text-[9px] leading-snug text-zinc-700 sm:px-1 sm:text-[10px]"
                            title={`${seg.from} 至 ${seg.to}`}
                          >
                            <span className="block whitespace-nowrap">{seg.from}</span>
                            <span className="block py-0.5 text-center text-[8px] text-zinc-400">↓</span>
                          ...
                          <td className="px-0.5 py-1.5 text-right font-mono text-zinc-600">
            ...
            <div id="overview-correlation">
              <div className="flex flex-wrap items-end justify-between gap-3">
                ...
                <label className="text-xs text-zinc-600">
                  ...
                  <select
                    ...
                    className="ml-2 rounded-md border border-zinc-200 bg-white px-2 py-1 text-xs text-zinc-800"
                  >
              ...
              <p className="mt-1 text-xs text-zinc-500">当前按<strong>{currentWindowLabel}</strong>取样；仅使用各标的<strong>日期交集</strong>上的日收益序列，重合不足 30 个交易日时不展示矩阵。</p>
              <p className="mt-1 text-[11px] text-zinc-500">热力底色：红色=正相关更强，蓝色=负相关更强；悬浮可看精确值。</p>
                  ...
                  <table className="w-full min-w-[240px] table-fixed text-center text-[11px] sm:text-xs">
                    ...
                    <thead>
                      <tr className="border-b border-zinc-100 bg-zinc-50">
                        ...
                        {corrOrderedLabels.map((lb) => (
                          <th key={lb} className="px-1 py-2 font-mono font-semibold text-zinc-600">
                            {lb}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {corrOrderedIndices.map((rowIdx) => (
                        <tr key={compareResult.corrLabels[rowIdx]} className="border-b border-zinc-50">
                          <td className="bg-zinc-50 px-1 py-2 font-mono font-semibold text-zinc-600">{compareResult.corrLabels[rowIdx]}</td>
