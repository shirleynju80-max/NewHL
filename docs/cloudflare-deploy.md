...
说明：`index-t1-sync` 默认会对恒生同步做最多 3 次重试（网络抖动自愈），并按**工作日**检测 `HSI114` / `HSSCSOY.HI` 是否陈旧（默认阈值 3 个工作日）。工作日有北京时间 18:35 / 20:35 两个窗口；18:35 失败或告警先不推送飞书/企业微信，20:35 后若当天任意一次定时同步已成功，则不再为单次失败告警；若当天仍无成功 run，才发送失败告警。手动触发仍按单次结果告警。
